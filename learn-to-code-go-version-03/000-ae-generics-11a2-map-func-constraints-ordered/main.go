...
func main() {
	...
	xi1 := []int{1, 2, 3}
	xi2 := mapVals(xi1, func(n int) int { return n * 2 })
	fmt.Println(xi1)
	fmt.Println(xi2)
	...
	xf2 := mapVals(xf1, func(f float64) float64 { return f * 2 })
	fmt.Println(xf1)
	fmt.Println(xf2)
}

// обмеження типів які підтримують операції порівняння, такі як int, float64, string
